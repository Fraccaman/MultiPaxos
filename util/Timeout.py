...
class Timeout:
    ...
    def __init__(self, node: Proposer):
        ...
        self.timer_leader_election = threading.Thread(target=self.heartbeat_leader)
        self.timer_phase_one.setDaemon(True)
        self.timer_phase_two.setDaemon(True)
        self.timer_leader_election.setDaemon(True)
    ...
    def timeout_phase_one_a(self):
        while True:
            if self.node.is_leader() and self.phase_one_time + 3 < time.time():
    ...
    def timeout_phase_two_a(self):
        while True:
            if self.node.is_leader() and self.phase_two_time + 3 < time.time():
    ...
    def heartbeat_leader(self):
        while True:
            if self.leader_election_time + 6 < time.time():
